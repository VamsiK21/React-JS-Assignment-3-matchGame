...
class App extends Component {
  ...
  getTheGame = () => {
    const {randomImageUrl, activeTabId} = this.state
    const filteredItems = this.getFilteredItems()
    return (
      <div className="content-container">
        <div className="result-img-container">
          <img className="result-image" src={randomImageUrl} alt="match" />
        </div>
        ...
        <ul className="tumbnails-container">
          {filteredItems.map(eachItem => (
            <ThumbnailItem
              onChangerandomImage={this.onChangerandomImage}
    ...
    return (
      <div className="score-card-container">
        <img
          ...
          alt="trophy"
        />
        <div className="bottom-score-btn-card">
          <p className="heading-score">YOUR SCORE</p>
          <p className="total-score-text">{score}</p>
          <button
            onClick={this.onClickedPlayAgain}
  ...
  render() {
    const {timer, score, scoreCard} = this.state

    return (
      <div className="bg-container">
        <NavBar timer={timer} score={score} />

        {scoreCard === true || timer === 0
          ? this.getScoreCard()
          : this.getTheGame()}
      </div>
    )
